...
def dictionary_cracking_with_salt(
    ...
    plaintext_pass = [None] * len(hashes)
    for index in range(len(hashes)):
        hashed_file = hash_file(dictionary, hashes[index][1])
        if hashes[index][0] in hashed_file:
            plaintext_pass[index] = hashed_file[hashes[index][0]]
...
if __name__ == "__main__":
    ...
    task1_values = brute_force_cracking(task1_hashed_vals)
    print(task1_values)
    ...
    task2_values = dictionary_cracking(task2_hashed_vals, "PasswordDictionary.txt")
    print(task2_values)
    ...
    task3_vals = dictionary_cracking_with_salt(
        task3_hashed_vals, "PasswordDictionary.txt"
    )
    print(task3_vals)
